...
describe('PubSub', () => {
  ...
  it.only('subscribe -> publish', async () => {
    let sum = 0;
    let val = random();
  ...
  it.only('subscribe -> publish -> unsubscribe -> publish', async () => {
    let sum = 0;
    let val = random();
    let add = val => (sum += val);
...
describe('Observable', () => {
  ...
  it.only('addObserver -> notify', async () => {
  ...
  it.only('addObserver -> notify -> removeObserver -> notify', async () => {
    let subject = new Observable.Subject();
    let ob = new Observer();
    subject.addObserver(ob);
    assert.ok(subject.observers.count() === 1);
